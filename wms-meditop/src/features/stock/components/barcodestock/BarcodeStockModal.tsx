...
import "./barcodestock.css"

type BarcodeStockModalProps = {
  isOpen: boolean;
  onClose: () => void;
...
const BarcodeStockModal = ({
  isOpen,
  onClose,
  stockItem,
}: BarcodeStockModalProps) => {
  ...
  useEffect(() => {
    if (isOpen && stockItem) {
      setLot(stockItem.lot_name || "");

      if (stockItem.expiration_date) {
        const expDate = new Date(stockItem.expiration_date);
        const formattedExp = expDate.toISOString().slice(0, 10); // YYYY-MM-DD
        setExp(formattedExp);
      } else {
        setExp("");
      }
        setNoExpiry(!stockItem.expiration_date);
    }
  }, [isOpen, stockItem]);
  ...
  const buildScanPayload = (item: StockType) => {
    const barcodeText =
    ...
    const lotPart =
      (item as any).lot_name && String((item as any).lot_name).trim()
        ? String((item as any).lot_name).trim()
        : "XXXXXX";

    // ✅ exp ถ้า null → 999999
    const expPart = item.expiration_date ? formatYYMMDD(item.expiration_date) : "999999";
  ...
  const handlePrint = () => {
    ...
    const qrPayload = buildScanPayload(stockItem);

    const lotText = lot || "XXXXXX";
    const expText =
      noExpiry || !exp ? "** No Expiry **" : formatDateOnlyDMY(exp);

    const productText = stockItem.product_code || "---";
...
export default BarcodeStockModal;
